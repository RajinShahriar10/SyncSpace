...
function RegisterForm() {
  ...
  const { register: registerUser, googleLogin, githubLogin, error, setError } = useAuthStore();
    ...
    if (clientId && typeof window !== "undefined") {
      ...
      script.onload = () => {
        if (window.google?.accounts?.id) {
          ...
          const googleBtn = document.getElementById("google-signin-btn-register");
          if (googleBtn) {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID ? (
                <div id="google-signin-btn-register" className="w-full" />
              ) : (
                <Button
                  variant="outline"
                  className="w-full"
